...
def main():
    ...
    # Wait for arm goal to complete
    rclpy.spin_until_future_complete(node, arm_future)
    if arm_future.result().accepted:
        node.get_logger().info("Arm goal accepted and completed.")
    else:
        node.get_logger().warn("Arm goal was not completed successfully.")

    # Wait for gripper goal to complete
    rclpy.spin_until_future_complete(node, gripper_future)
    if gripper_future.result().accepted:
        node.get_logger().info("Gripper goal accepted and completed.")
    else:
        node.get_logger().warn("Gripper goal was not completed successfully.")
