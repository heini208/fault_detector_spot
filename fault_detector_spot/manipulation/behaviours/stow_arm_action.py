"""Thin behavior-tree adapter for ArmMovementExecutor.stow()."""
...
class StowArmActionSimple(py_trees.behaviour.Behaviour):
    """Expose the executor's stow operation to the command tree."""
    ...
    def setup(self, **kwargs):
        ...
        if self.node is None:
            raise RuntimeError(
                "StowArmActionSimple requires a ROS node"
            )
    ...
    def update(self) -> Status:
        try:
            ...
            if self._started:
                ...
                update = self.arm_movement_executor.stow()
    ...
    def _ensure_executor(self) -> None:
        ...
        if self.robot_command_resources is None:
            raise RuntimeError(
                "StowArmAction requires shared robot command resources"
            )
    ...
    def _fail(self, detail: str) -> Status:
        if (
            ...
            self.arm_movement_executor.cancel()
        self._started = False
        normalized = str(detail).strip() or "Stow arm failed"
