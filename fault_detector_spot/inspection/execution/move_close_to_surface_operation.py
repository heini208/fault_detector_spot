"""Execute one force-guarded close-surface movement."""
...
class MoveCloseToSurfaceOperation:
    ...
    def __init__(
        ...
        if node is None:
            raise RuntimeError("MoveCloseToSurfaceOperation requires a ROS node")
        if state_source is None:
            raise RuntimeError("MoveCloseToSurfaceOperation requires runtime state")
        if robot_command_client is None:
            raise RuntimeError("MoveCloseToSurfaceOperation requires a robot client")
        self.name = name
        self.node = node
        self._state_source = state_source
        self._client = robot_command_client
    ...
    def start(self, command: MoveCloseToSurfaceRequest) -> None:
        """Start one operation with an immutable command snapshot."""
        if not isinstance(command, MoveCloseToSurfaceRequest):
            raise TypeError("Expected a MoveCloseToSurfaceRequest")
        self._clear_runtime()
    ...
    def cancel(self) -> None:
        """Cancel active robot motion and invalidate this operation."""
        self._cancel_active_goal()
        self._clear_runtime()

    def update(self) -> MoveCloseToSurfaceStatus:
        """Advance the non-blocking operation state machine once."""
        if self._command is None:
            return self._fail("Close-surface operation was not started")
        try:
    ...
    @property
    def phase(self) -> str:
        """Return the current execution phase for ROS feedback."""
        return self._phase

    def _update_acquire(self) -> MoveCloseToSurfaceStatus:
        try:
    ...
    def _update_sampling(self) -> MoveCloseToSurfaceStatus:
        ...
        try:
            ...
            if self._plan.initial_axis_error_rad > self.maximum_axis_error_rad:
                raise ValueError(
    ...
    def _update_moving(self) -> MoveCloseToSurfaceStatus:
        self._require_attachment_unchanged()
    ...
    def _update_settling(self) -> MoveCloseToSurfaceStatus:
        self._require_attachment_unchanged()
        force_error = self._check_force_guard()
        if force_error is not None:
            return self._begin_motion_recovery(
        ...
        self.feedback_message = (
            ...
            f"estimated distance {evaluation.estimated_distance_m:.4f} m"
        )
        return self._prepare_next_approach_step()
    ...
    def _update_recovering(self) -> MoveCloseToSurfaceStatus:
        result = self._poll_goal()
        if result is None:
            return MoveCloseToSurfaceStatus.RUNNING
        if result is False:
