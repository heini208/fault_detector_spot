...
class PoseData:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PoseData":
        ...
        return cls(
            ...
            orientation=QuaternionData.from_dict(data["orientation"]),
        )

    @classmethod
    def identity(cls) -> "PoseData":
        return cls(position=Vector3Data.zero(), orientation=QuaternionData.identity())
...
class ImagePoint:
    ...
    def validate(self) -> None:
        if self.u < 0 or self.v < 0:
            raise ValueError("Reference pixel coordinates must not be negative")
...
class ReferenceTag:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ReferenceTag":
        ...
        return cls(tag_id=int(data["tag_id"]), tag_family=str(data["tag_family"]))
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"tag_id": self.tag_id, "tag_family": self.tag_family}
...
class ReferenceView:
    ...
    def validate(self) -> None:
        ...
        _require_text(self.controlled_frame, "Reference view controlled frame")
        if self.reference_dataset_path is not None and not self.reference_dataset_path.strip():
            raise ValueError("Reference dataset path must not be empty")
        identity = (self.view_id, self.camera_id, self.slot_index)
        if any(value is not None for value in identity):
            if any(value is None for value in identity):
                raise ValueError("Reference view identity must be complete")
            ...
            if (
                ...
                raise ValueError("Reference view slot must be between 0 and 2")
        if self.reference_dataset_path is not None and self.view_id is None:
            raise ValueError("Stored reference view requires camera identity")

    def to_dict(self) -> Dict[str, Any]:
        result = {
            "controlled_frame_pose_object": self.controlled_frame_pose_object.to_dict(),
            "controlled_frame": self.controlled_frame,
        }
        if self.reference_dataset_path is not None:
            result["reference_dataset_path"] = self.reference_dataset_path
...
class ProbePoint:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ProbePoint":
        ...
        return cls(
            ...
            safe_approach_pose_object=PoseData.from_dict(data["safe_approach_pose_object"]),
            probe_pose_object=PoseData.from_dict(data["probe_pose_object"]),
            target_surface_distance_m=float(data["target_surface_distance_m"]),
            position_tolerance_m=float(data["position_tolerance_m"]),
            orientation_tolerance_rad=float(data["orientation_tolerance_rad"]),
            measurement_duration_sec=float(data["measurement_duration_sec"]),
            aligned_preapproach_distance_m=float(data["aligned_preapproach_distance_m"]),
            reference_pixel=(ImagePoint.from_dict(pixel) if pixel is not None else None),
            reference_view_id=(str(reference_view_id) if reference_view_id is not None else None),
            sensor_path=(str(sensor_path) if sensor_path is not None else None),
        )
    ...
    def validate(self) -> None:
        ...
        if not all(math.isfinite(value) for value in numeric_values):
            raise ValueError("Probe point contains a non-finite numeric value")
        ...
        if self.reference_pixel is not None:
            ...
            if self.reference_view_id is None:
                raise ValueError("Reference pixel requires a reference view ID")
        if self.reference_view_id is not None:
            _require_text(self.reference_view_id, "Probe point reference view ID")
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "safe_approach_pose_object": self.safe_approach_pose_object.to_dict(),
            ...
            "aligned_preapproach_distance_m": self.aligned_preapproach_distance_m,
        }
        if self.reference_pixel is not None:
            result["reference_pixel"] = self.reference_pixel.to_dict()
        if self.reference_view_id is not None:
...
class InspectionRoutine:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "InspectionRoutine":
        data = _require_dict(data, "inspection_routine")
        probe_points = _require_list(data["probe_points"], "probe_points")
        reference_views = _require_list(data["reference_views"], "reference_views")
        return cls(
            routine_id=str(data["routine_id"]),
            display_name=str(data["display_name"]),
            sensor_id=str(data["sensor_id"]),
            reference_views=[ReferenceView.from_dict(view) for view in reference_views],
            probe_points=[ProbePoint.from_dict(point) for point in probe_points],
        )
    ...
    def validate(self) -> None:
        ...
        if not 0 <= len(self.reference_views) <= 3:
            raise ValueError("Routine must contain at most three reference views")
        ...
        for reference_view in self.reference_views:
            ...
            if reference_view.reference_dataset_path is None:
                raise ValueError("Routine reference view requires a dataset path")
            if reference_view.view_id is None:
                raise ValueError("Routine reference view requires an ID")
            if reference_view.camera_id is None:
                raise ValueError("Routine reference view requires a camera ID")
            if reference_view.slot_index is None:
                raise ValueError("Routine reference view requires a slot")
            if reference_view.view_id in view_ids:
                raise ValueError(f"Duplicate reference view ID: {reference_view.view_id}")
            if reference_view.camera_id in camera_ids:
                raise ValueError(f"Duplicate reference camera ID: {reference_view.camera_id}")
            if reference_view.slot_index in slot_indices:
                raise ValueError(f"Duplicate reference view slot: {reference_view.slot_index}")
        ...
        if not self.reference_views and self.probe_points:
            raise ValueError("Routine with probe points requires a reference view")
        ...
        for probe_point in self.probe_points:
            ...
            if probe_point.probe_point_id in probe_ids:
                raise ValueError(f"Duplicate probe point ID: {probe_point.probe_point_id}")
            probe_ids.add(probe_point.probe_point_id)

    def get_probe_point(self, probe_point_id: str) -> Optional[ProbePoint]:
        return next(
            (point for point in self.probe_points if point.probe_point_id == probe_point_id),
            None,
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "reference_views": [view.to_dict() for view in self.reference_views],
            "probe_points": [point.to_dict() for point in self.probe_points],
        }
...
class InspectionObject:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "InspectionObject":
        ...
        return cls(
            ...
            routines=[InspectionRoutine.from_dict(routine) for routine in routines],
        )

    def validate(self) -> None:
        _require_text(self.object_id, "Inspection object ID")
        _require_text(self.display_name, "Inspection object display name")
        self.reference_tag.validate()
        routine_ids: Set[str] = set()
        for routine in self.routines:
            routine.validate()
            if routine.routine_id in routine_ids:
                raise ValueError(f"Duplicate routine ID: {routine.routine_id}")
            routine_ids.add(routine.routine_id)

    def get_routine(self, routine_id: str) -> Optional[InspectionRoutine]:
        return next(
            (routine for routine in self.routines if routine.routine_id == routine_id),
            None,
        )
