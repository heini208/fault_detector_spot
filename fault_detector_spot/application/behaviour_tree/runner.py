...
from fault_detector_spot.application.behaviour_tree.behaviours.reset_estop_flag import (
    ResetEstopFlag,
)
...
from fault_detector_spot.manipulation.behaviours.arm_movement_action import (
    ArmMovementAction,
)
...
from fault_detector_spot.navigation.behaviours.cancel_movement import (
    PublishZeroVel,
)
...
def build_sensing_tree(node: rclpy.node.Node) -> py_trees.behaviour.Behaviour:
    ...
    cmd_sub = CommandSubscriber(
        name="UI Command Listener", defer_subscription=True,
    )
...
def main(args=None):
    ...
    node.get_logger().info(
        f"BT node initialized in {time.monotonic() - started:.3f}s"
    )
    root = create_root(node)
    node.get_logger().info(
        f"BT root constructed at +{time.monotonic() - started:.3f}s"
    )
    ...
    try:
        ...
        for behaviour in root.iterate():
            if isinstance(behaviour, CommandSubscriber):
                behaviour.activate()
        node.get_logger().info(
            f"BT command consumer ready at +{time.monotonic() - started:.3f}s"
        )
        executor.spin()
