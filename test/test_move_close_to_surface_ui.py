...
def test_tag_surface_test_payload_uses_xyz_in_tag_frame():
    source = _controls_source()
    ...
    assert 'self._get_offset("X")' in handler
...
def test_tag_surface_info_explains_temporary_input_semantics():
    ...
    assert "Arm offset X/Y/Z" in source
    assert "interpreted in the tag frame" in source
    assert "lies behind the physical surface" in source
    assert "Distance (m) sets the final probe-tip stand-off" in source
    assert "Tolerance (m) sets the allowed final distance error" in source
    assert "X backward/forward offset does not define the final stand-off" in source
